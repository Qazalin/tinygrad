...
y.numpy().tofile("y.bin")
z.numpy().tofile("z.bin")
"""


ts = [torch.from_numpy(np.fromfile(x, dtype=np.float32)).reshape(32,8,16,64) for x in ["x.bin", "y.bin", "z.bin"]]
tst = [Tensor(x.detach().numpy(), requires_grad=True) for x in ts]
x, y, z = tst
out = Tensor.scaled_dot_product_attention(x,y,z,is_causal=True)
print(out.numpy()[0][0][0])
