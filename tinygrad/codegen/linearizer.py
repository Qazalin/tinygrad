...
class Linearizer(Kernel):
  ...
  def linearize(self):
    ...
    if self.group_for_reduces:
      ...
      self.bufs.append(MemBuffer(idx:=len(self.bufs), temp_dtype, self.sts[-1]))
      self.buf_uops.append(self.uop(UOps.DEFINE_LOCAL, PtrDType(temp_dtype), (), (f"ldata{idx}", self.sts[-1].size)))
