...
class Sigmoid(Function):
  def forward(self, x:LazyBuffer) -> LazyBuffer:
    self.ret = x.cast(ftype:=least_upper_float(x.dtype)).const(1).e(
      BinaryOps.DIV, x.cast(ftype).const(1).e(BinaryOps.ADD, x.cast(ftype).e(BinaryOps.MUL, x.cast(ftype).const(-1/math.log(2))).e(UnaryOps.EXP2)))
    return self.ret
...
class Less(Function):
  def forward(self, x:LazyBuffer, y:LazyBuffer) -> LazyBuffer:
    return x.cast(out_dtype:=least_upper_dtype(x.dtype, y.dtype)).e(BinaryOps.CMPLT, y.cast(out_dtype))
...
class Sum(Function):
  def forward(self, x:LazyBuffer, new_shape:Tuple[int, ...]) -> LazyBuffer:
    ...
    if x.dtype == dtypes.bool: x = x.cast(dtypes.int32)
...
class Max(Function):
  ...
  def backward(self, grad_output:LazyBuffer) -> LazyBuffer:
    ...
    max_is_1s = self.x.const(1.0).e(BinaryOps.SUB, self.x.e(BinaryOps.CMPLT, self.ret.expand(self.x.shape)).cast(grad_output.dtype))
