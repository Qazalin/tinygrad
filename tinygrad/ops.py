...
InterpretedFlopCounter: Dict[Op, Callable] = {
  ...
  **{op:lambda self,y: FlopCounter(self.shape, dtypes.bool if op == BinaryOps.CMPLT else least_upper_dtype(self.dtype, y.dtype), self.consume_flops() + y.consume_flops() + prod(self.shape), {**self.mem, **y.mem}) for op in BinaryOps},  # noqa: E501
