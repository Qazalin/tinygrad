...
const drawGraph = (data, opts) => {
  ...
  let labels = nodes.selectAll("g.label").data(d => [d]).join("g").attr("class", "label");
  if (!opts?.labelAuto) labels = labels.attr("transform", d => `translate(-${d.labelWidth/2}, -${d.labelHeight/2+STROKE_WIDTH*2})`);
  ...
  if (opts?.labelAuto) labels.attr("transform", (_,i,els) => {
...
function renderDag(graph, additions, recenter) {
  ...
  worker.onmessage = (e) => {
    ...
    addTags(d3.select("#edge-labels").selectAll("g.edge").data(e.data.edges).join("g").attr("transform", (e) => {
    ...
    }).attr("class", e => e.value.label.type ?? "edge-label").attr("id", e => `${e.v}-${e.w}`).datum(e => e.value.label.text));
...
const ncu_layout = (counters) => {
  ...
  // currently ncu just adds them all up
  const formulas = {
...
async function main() {
  ...
  if (ckey.startsWith("/render")) {
    ...
    if (ckey.includes("mem")) return renderCacheGraph(ret);
