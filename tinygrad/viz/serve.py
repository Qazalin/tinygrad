...
def get_ts_diff(device:str, thread=0) -> Decimal: return diff[thread] if (diff:=device_ts_diffs.get(device)) else Decimal(0)

DevEvent = ProfileRangeEvent|ProfileGraphEntry|ProfilePointEvent
def flatten_events(profile:list[ProfileEvent]) -> Generator[tuple[Decimal, Decimal|None, DevEvent], None, None]:
  for e in profile:
    if isinstance(e, ProfileRangeEvent): yield (e.st+(ts_diff:=get_ts_diff(e.device, e.is_copy)), (e.en or e.st)+ts_diff, e)
    if isinstance(e, ProfilePointEvent): yield (e.st, None, e)
    if isinstance(e, ProfileGraphEvent):
      cpu_ts:list[Decimal] = []
      for ent in e.ents: cpu_ts += [e.sigs[ent.st_id]+(ts_diff:=get_ts_diff(ent.device, ent.is_copy)), e.sigs[ent.en_id]+ts_diff]
...
def get_profile(profile:list[ProfileEvent]):
  ...
  for ts,en,e in flatten_events(profile):
    dev_events.setdefault(e.device,[]).append((st:=int(ts), et:=(st if en is None else int(en)), 0. if en is None else float(en-ts), e))
