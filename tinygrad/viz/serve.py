...
def flatten_events(profile:list[ProfileEvent], devs) -> Generator[tuple[decimal.Decimal, decimal.Decimal|None, DevEvent], None, None]:
  for e in profile:
    ...
    if isinstance(e, ProfileGraphEvent):
      st = e.sigs[e.ents[0].st_id]-devs[e.ents[0].device][e.ents[0].is_copy]
      et = e.sigs[e.ents[-1].en_id]-devs[e.ents[-1].device][e.ents[-1].is_copy]
      yield (st, et, ProfileRangeEvent(f"{e.ents[0].device} Graph", f"batched {len(e.ents)}", st, et))
