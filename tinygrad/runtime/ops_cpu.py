...
class ClangJITCompiler(Compiler):
  def __init__(self, cachekey="compile_clang_jit"):
    ...
    self.target, self.march = ('x86_64' if sys.platform == 'win32' else platform.machine())+'-none-unknown-elf', 'native'
    super().__init__(cachekey)

  def compile(self, src:str) -> bytes:
    # -fno-math-errno is required for __builtin_sqrt to become an instruction instead of a function call
    args = [f'-march={self.march}', f'--target={self.target}', '-O2', '-fPIC', '-ffreestanding', '-fno-math-errno', '-nostdlib', '-fno-ident']
    arch_args = ['-ffixed-x18'] if self.target == 'arm64' else []
