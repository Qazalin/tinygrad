...
from tinygrad.helpers import DEBUG, diskcache, from_mv, init_c_var, compile_cuda_style, encode_args_cuda_style, time_execution_cuda_style
...
class HIPDevice(Compiled):
  ...
  def __init__(self, device:str=""):
    ...
    if self.device == 0: HIPDevice.default_arch_name = init_c_var(hip.hipDeviceProp_t(), lambda x: check(hip.hipGetDeviceProperties(x, self.device))).gcnArchName.decode()

    from tinygrad.features.graph.hip import HIPGraph
    super().__init__(HIPAllocator(self.device), LinearizerOptions(device="HIP"), HIPRenderer, compile_hip, functools.partial(HIPProgram, self.device), HIPGraph)
