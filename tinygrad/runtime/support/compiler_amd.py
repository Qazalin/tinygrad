...
class AMDLLVMCompiler(LLVMCompiler):
  ...
  def compile(self, src:str) -> bytes:
    if src.split('\n', 1)[0].strip() == '.text':
      try:
        return subprocess.check_output(["clang", "-c", "-x", "assembler", "--target=amdgcn-amd-amdhsa", f"-mcpu={self.arch}", "-o", "-", "-"],
                                       input=src.encode("utf-8"))
      except subprocess.CalledProcessError as e:
        raise CompileError(e.output.decode("utf-8", "ignore") if e.output else str(e)) from e
