...
class UOp(RandMixin, metaclass=UOpMetaClass):
  ...
  def addrspace(self) -> AddrSpace|None:
    ...
    if self.op in {Ops.INDEX, Ops.CAST, Ops.AFTER, Ops.REDUCE, Ops.GEP, Ops.STORE, Ops.MSTACK, Ops.MSELECT, Ops.SLICE}:
  ...
  def buf_uop(self) -> UOp:
    ...
    s = self
    while len(s.src) and (s.op not in {Ops.BUFFER, Ops.PARAM, Ops.STAGE, Ops.MSTACK, Ops.SLICE} or (s.op is Ops.SLICE and s.src[0].op is Ops.INDEX)): s = s.src[0]
    return s
