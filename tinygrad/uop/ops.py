...
def dtype_from_uop(op:Ops, src:tuple[UOp,...], arg:Any) -> DType|None:
  ...
  match op:
    ...
    case Ops.NOOP:
      ...
      return None
    case Ops.LOAD | Ops.INDEX | Ops.UNSHARD | Ops.REDUCE | Ops.LOCAL_GRAD | Ops.AFTER | Ops.RANGE | \
...
class UOp(RandMixin, metaclass=UOpMetaClass):
  ...
  def _shape(self) -> tuple[sint, ...]|None:
    ...
    if self.op in GroupOp.Movement.union({Ops.UNSHARD, Ops.REDUCE, Ops.LOCAL_GRAD}):
      ps = self.src[0]._shape
...
class ProgramInfo:
  ...
  def from_sink(sink:UOp, target:Target=Target()) -> ProgramInfo:
    ...
    for u in sink.toposort():
      ...
      if u.op is Ops.CUSTOM and u.dtype is dtypes.void and u.src and u.src[0].op is Ops.INDEX:
