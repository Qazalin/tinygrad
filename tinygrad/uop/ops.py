...
def track_rewrites(name:Callable[..., str|TracingKey]|bool=True, replay:bool=False):
  def _decorator(func):
    def __wrapper(*args, **kwargs):
      fn = func.__name__
      key = TracingKey(n:=f"{fn} n{next(_name_cnt.setdefault(fn, itertools.count(1)))}", (n,))
      ...
      if TRACK_MATCH_STATS >= 2 and callable(name):
        ...
        e.name = TracingKey(n:=e.name.display_name.replace(fn, name_ret), (n,)) if isinstance(name_ret, str) else name_ret
