...
class CStyleLanguage(NamedTuple):
  ...
  def render_cast(self, x:List[str], var_dtype:DType) -> str:
    ...
    if var_dtype.scalar() == dtypes.half: return f"{{{','.join(f'(half){x}' for x in x)}}}" # TODO do we really need this?
  ...
  def render_kernel(self, function_name:str, kernel:List[str], bufs:List[Tuple[str,DType]], local_size:List[int], prekernel:List[str]) -> str:
    ...
    [") {\n" + tmp] + ['\n'.join(kernel), "\n}"])
    if self.half_prekernel and any(dtype.scalar() == dtypes.half if not isinstance(dtype, ImageDType) else dtype.underlying == dtypes.half for _,dtype in bufs): prg = ''.join([f"{self.half_prekernel}", "\n", prg])
    return prg
  ...
  for u in uops:
    ...
    if uop == UOps.LOOP:
      ...
      if vin[0].uop == UOps.ALU and vin[0].arg == args and args in {BinaryOps.ADD, BinaryOps.SUB, BinaryOps.MUL}:
        ...
        val = lang.code_for_op[args](*[lang.render_cast([r[x]], dtype) for x in vin])
      else:
