...
class CStyleLanguage(NamedTuple):
  ...
  def render_const(self, x:Union[float,int,bool], var_dtype) -> str:
    ...
    return self.render_cast([val]*var_dtype.sz, var_dtype) if var_dtype.sz > 1 or (dtypes.is_int(var_dtype) and var_dtype != dtypes.int) else val
  ...
  def render_store(self, buf_name:str, buf_dtype:DType, var_name:str, var_dtype:DType, idx:str, local=False) -> str:
    ...
    if var_dtype.sz > 1:
      idxs = [f"{'xyzw'[i]}" for i in range(var_dtype.sz)]
      var = self.render_cast([f"{var_name}.{idxs[i]}" for i in range(var_dtype.sz)], buf_dtype.vec(var_dtype.sz))
      return f"*(({self.smem_prefix if local and self.smem_prefix_for_cast else self.buffer_prefix}{buf_dtype.name}{var_dtype.sz}*)({buf_name}+{idx})) = {var};"
