...
def view_to_slice(r:UOp) -> UOp|None:
  ...
  return UOp(Ops.SLICE, r.dtype, (src, UOp.const(offset, dtypes.weakint)), size)
...
def contiguous_index_source(idx:UOp, shape:tuple[sint, ...], ranges:tuple[UOp, ...]) -> UOp|None:
  ...
  linear_idx, stride = UOp.const(0, dtypes.weakint), cast(sint, 1)
  for size, ridx in reversed(tuple(zip(shape, ranges))): linear_idx, stride = linear_idx + ridx * stride, stride * size
  ...
  return idx.src[0].reshape(shape) if term_keys(idx.src[1]) == term_keys(linear_idx) else None
...
def remove_noop_mselect_bufferize(m:UOp):
  ...
  live_idxs = tuple(x for s,x in zip(idx.src[0].shape, idx.src[1:]) if not (x.op is Ops.CONST and x.val == 0 and resolve(s == 1)))
...
def bufferize_to_store(ctx:itertools.count, x:UOp, idx:UOp, allow_locals=True):
  ...
  # A full identity CONTIGUOUS after an opaque PROGRAM is only a scheduling boundary. Reuse its output buffer.
  ...
  if source is not None and source.op is Ops.INDEX and is_program_output(source.src[0]):
    ...
    if all(r.src[0].op is Ops.CONST for r in source_rngs):
      source_shape = tuple(r.src[0].val for r in source_rngs)
  ...
  if x.arg.addrspace == AddrSpace.GLOBAL:
    ...
    do_store = buf.store(x.src[0].cast(dtype)).end(*rngs) if x.src[0].op is Ops.SLICE else buf.index(idx).store(x.src[0].cast(dtype)).end(*rngs)
