...
def create_schedule(sched_sink:UOp) -> UOp:
  with cpu_profile(TracingKey("toposort sched_sink")):
    ...
    program_outputs: dict[UOp, set[UOp]] = {}  # opaque PROGRAM call -> buffers whose AFTER state keeps the write
  ...
  with cpu_profile(TracingKey("linearize schedule")):
    ...
    def distance_to_precompiled(root:UOp) -> int|None:
      ...
      while stack:
        ...
        if is_precompiled(node): linear_distance[node] = 0
          ...
          linear_distance[node] = 1 + min(distances) if distances else None
        else:
    ...
    linearized: list[UOp] = []
    while queue or continuation:
      ready_group = [x for x in queue if x in overlap_group]
      if drain_group and ready_group:
        rk = ready_group[0]
        queue.remove(rk)
        ...
        reachable = [x for x in continuation if distance_to_precompiled(x) is not None]
        stop_at_precompiled = bool(reachable)
        overlap_limit, overlap_steps, overlapping = len(copy_parents[queue[0]]), 0, True
        rk = min(reachable, key=lambda x: distance_to_precompiled(x) or 0) if stop_at_precompiled else max(continuation, key=downstream_path)
        continuation.remove(rk)
      elif continuation and (not queue or queue[0] not in copy_children): rk = continuation.popleft()
      ...
      for x in children.get(rk, []):
        in_degree[x] -= 1
        if in_degree[x] == 0:
          if x not in copy_children and queue and queue[0] in copy_children: continuation.append(x)
          else: queue.append(x)
      if overlapping:
        overlap_steps += 1
        if is_precompiled(rk) or not continuation or (not stop_at_precompiled and overlap_steps >= overlap_limit):
