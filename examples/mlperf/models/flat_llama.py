...
def matmul(x:Tensor, w:Tensor, fp8:bool=True, amax_x:Tensor|None=None, w_inv_scale:Tensor|None=None,
           ...
           grad_amax_state:Tensor|None=None, w_scale_offset:int=0) -> tuple[Tensor,...]:
  if not fp8:
    if ASM_GEMM:
  ...
  if ASM_GEMM:
    ...
    if can_use_asm_gemm(x_fp8, w.T):
      ...
      if COLUMNWISE_WEIGHT_SCALE:
        out = asm_gemm(x_fp8, w.T, x_scale=gemm_x_scale, grad_amax_state=grad_amax_state, w_post_scale=w_inv_scale,
                       x_scale_is_amax=x_scale_is_amax)
      else:
        out = asm_gemm(x_fp8, w.T, x_scale=gemm_x_scale, w_scale=w_inv_scale, grad_amax_state=grad_amax_state,
                       x_scale_is_amax=x_scale_is_amax, w_scale_offset=w_scale_offset)
      return out, x_new_amax, x_fp8
  return (x_fp8.dot(w.T, dtype=dtypes.float) * x_scale * (w_inv_scale[w_scale_offset] if w_inv_scale.ndim else w_inv_scale)).cast(dtypes.bfloat16), x_new_amax, x_fp8

def norm_quantize_matmul(x:Tensor, norm:Tensor, w:Tensor, w_inv_scale:Tensor, eps:float, amax_x:Tensor, grad_amax_state:Tensor, w_scale_offset:int=0):
  if FUSED_ADD_NORM_MUL_QUANTIZE:
    ...
    out, *ret = matmul(None, w, amax_x=amax_x, w_inv_scale=w_inv_scale, x_fp8=x_fp8, x_scale=x_inv_scale,
                       x_new_amax=new_amax, grad_amax_state=grad_amax_state, w_scale_offset=w_scale_offset)
    return out, x_normed, rrms, ret
  x_normed, rrms = rmsnorm(x, eps)
  out, *ret = matmul(x_normed * norm, w, amax_x=amax_x, w_inv_scale=w_inv_scale, grad_amax_state=grad_amax_state, w_scale_offset=w_scale_offset)
  return out, x_normed, rrms, ret

def add_norm_quantize_matmul(x:Tensor, residual:Tensor, norm:Tensor, w:Tensor, w_inv_scale:Tensor, eps:float, amax_x:Tensor,
                             grad_amax_state:Tensor|None=None, w_scale_offset:int=0):
  if FUSED_ADD_NORM_MUL_QUANTIZE:
...
def silu_w13_quantize_matmul(x_w13:Tensor, w2:Tensor, s_2:Tensor,
                             amax_x2:Tensor,
                             grad_amax_xw13:Tensor, grad_amax_xout:Tensor, w_scale_offset:int=0):
  if FUSED_SILU_W13:
    ...
    out, *ret = matmul(None, w2, amax_x=amax_x2, w_inv_scale=s_2, x_fp8=x2_fp8, x_scale=x2_inv_scale,
                       x_new_amax=new_amax_x2, grad_amax_state=grad_amax_xout, w_scale_offset=w_scale_offset)
    return out, ret
  hidden = x_w13.shape[-1] // 2
  x_w1, x_w3 = x_w13[..., :hidden], x_w13[..., hidden:]
  out, *ret = matmul(x_w1.silu() * x_w3, w2, amax_x=amax_x2, w_inv_scale=s_2, grad_amax_state=grad_amax_xout, w_scale_offset=w_scale_offset)
  return out, ret
...
class FlatTransformer:
  ...
  def attention(self, x:Tensor, freqs_cis:Tensor, *, attention_norm:Tensor, wqkv:Tensor, wo:Tensor,
                ...
                grad_amax_xqkv:Tensor, grad_amax_xo:Tensor, layer_idx:int):
    bsz, seqlen, _ = x.shape
    amaxs, saves = [], []

    xqkv, x_normed, rrms, (new_amax, *s) = norm_quantize_matmul(x, attention_norm, wqkv, s_qkv, self.norm_eps,
                                                                  amax_x=amax_xqkv, grad_amax_state=grad_amax_xqkv, w_scale_offset=layer_idx)
    ...
    out, new_amax, *s = matmul(attn, wo, amax_x=amax_xo, w_inv_scale=s_o, grad_amax_state=grad_amax_xo, w_scale_offset=layer_idx)
    amaxs.append(new_amax)
    saves.extend([*s, out])
  ...
  def feed_forward(self, x:Tensor, residual:Tensor, **kwargs):
    ...
    if SPLIT_W13:
      ...
      x_w1, new_amax, *s = matmul(inp, kwargs["w1"], amax_x=kwargs["amax_x1"], w_inv_scale=kwargs["s_1"], grad_amax_state=kwargs["grad_amax_xw1"], w_scale_offset=kwargs["layer_idx"])
      amaxs.append(new_amax)
      saves.extend([*s, x_w1])
      x_w3, new_amax, *s = matmul(inp, kwargs["w3"], amax_x=kwargs["amax_x3"], w_inv_scale=kwargs["s_3"], grad_amax_state=kwargs["grad_amax_xw3"], w_scale_offset=kwargs["layer_idx"])
      amaxs.append(new_amax)
      saves.extend([*s, x_w3])
      out, new_amax, *s = matmul(x_w1.silu() * x_w3, kwargs["w2"], amax_x=kwargs["amax_x2"], w_inv_scale=kwargs["s_2"],
                                 grad_amax_state=kwargs["grad_amax_xout"], w_scale_offset=kwargs["layer_idx"])
      amaxs.append(new_amax)
      saves.extend([*s, out])
    else:
      x_w13, h, x_normed, rrms, (new_amax, *s) = add_norm_quantize_matmul(x, residual, kwargs["ffn_norm"], kwargs["w13"], kwargs["s_13"],
                                                                          ...
                                                                          grad_amax_state=kwargs["grad_amax_xw13"], w_scale_offset=kwargs["layer_idx"])
      amaxs.append(new_amax)
      ...
      out, (new_amax, *s) = silu_w13_quantize_matmul(x_w13, kwargs["w2"], kwargs["s_2"], amax_x2=kwargs["amax_x2"],
                                                     grad_amax_xw13=kwargs["grad_amax_xw13"], grad_amax_xout=kwargs["grad_amax_xout"], w_scale_offset=kwargs["layer_idx"])
      amaxs.append(new_amax)
      saves.extend([*s, out])
  ...
  def __call__(self, tokens:Tensor, save:bool=True):
    ...
    for i in range(self.n_layers):
      attn_kwargs = dict(attention_norm=self.attention_norm[i], wqkv=self.wqkv[i], wo=self.wo[i],
                         amax_xqkv=a["xqkv"][i], amax_xo=a["xo"][i], s_qkv=s["wqkv"], s_o=s["wo"],
                         grad_amax_xqkv=ga["xqkv"][i], grad_amax_xo=ga["xo"][i], layer_idx=i)
      ffn_kwargs = dict(ffn_norm=self.ffn_norm[i], w2=self.w2[i],
                        amax_x2=a["x2"][i], s_2=s["w2"], grad_amax_xout=ga["xout"][i], layer_idx=i)
      if SPLIT_W13:
        ffn_kwargs.update(w1=self.w1[i], w3=self.w3[i], amax_x1=a["x1"][i], amax_x3=a["x3"][i],
                          s_1=s["w1"], s_3=s["w3"], grad_amax_xw1=ga["xw1"][i], grad_amax_xw3=ga["xw3"][i])
      else:
        ffn_kwargs.update(w13=self.w13[i], amax_x13=a["x13"][i], s_13=s["w13"], grad_amax_xw13=ga["xw13"][i])
