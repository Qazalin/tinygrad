...
class GradAccClipAdamW(Optimizer):
  ...
  def fstep(self, grads:list[Tensor]):
    ...
    fp8_inv_scales = [x for tt in self.params if hasattr(tt, '_inv_scale') for x in (tt._inv_scale if isinstance(tt._inv_scale, list) else [tt._inv_scale])]
    fp8_next_inv_scales = [x for tt in self.params if hasattr(tt, '_next_inv_scale') for x in (tt._next_inv_scale if isinstance(tt._next_inv_scale, list) else [tt._next_inv_scale])]
