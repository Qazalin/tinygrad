...
static void dump_params_or_extra(void** kernelParams, void** extra) {
  const void* argbuf = nullptr;
  size_t argbuf_size = 0;
  ...
  if (extra) {
    for (int i = 0; extra[i] != HIP_LAUNCH_PARAM_END; i += 2) {
      if (extra[i] == HIP_LAUNCH_PARAM_BUFFER_POINTER) {
        ...
        if (extra[i + 1]) argbuf_size = *(size_t*)extra[i + 1];
      }
    }
  }

if (argbuf && argbuf_size) {
  // kname isn't known here; callers can dump again with name if they want.
  dump_bytes_to_file("args_extra", "packed", argbuf, argbuf_size);
}
...
extern "C" hipError_t hipModuleLaunchKernel(hipFunction_t f,
  ...
  dump_params_or_extra(kernelParams, extra);
