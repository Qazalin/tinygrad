...
class TestMXFP4(unittest.TestCase):
  ...
  def test_swiglu_gemm(self):
    ...
    out, a_col, scale_a_col = asm_gemm(swiglu(x), w.T, mxfp4=True, return_mxfp4_saves=True)
    x1, x3 = x_ref[..., :256].float(), x_ref[..., 256:].float()
    ref = asm_gemm((x1.silu() * x3).cast(dtypes.bfloat16), w_ref.T, mxfp4=True)
    out.sum().backward()
    ref.sum().backward()
    Tensor.realize(out, a_col, scale_a_col, ref, x.grad, x_ref.grad, w.grad, w_ref.grad)
