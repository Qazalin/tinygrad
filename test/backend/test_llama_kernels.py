...
def run_fused_ce(bs:int, seqlen:int, vocab:int, label_smoothing:float=0.0) -> None:
  ...
  with Context(DEBUG=0):
    import numpy as np
    np.testing.assert_allclose(loss.numpy(), ref.numpy(), atol=2e-3, rtol=2e-3)
    #assert logits.grad.allclose(logits_ref.grad, atol=2e-3, rtol=2e-3).item(), "grad mismatch"

@unittest.skipUnless(Device.DEFAULT == "AMD", "only on amd because kernel source is HIP C")
class TestFusedCE(unittest.TestCase):
  def test_fused_ce_1_2_1024(self): run_fused_ce(1, 2, 1024)
  def test_fused_ce_2_16_1024(self): run_fused_ce(2, 16, 1024)
