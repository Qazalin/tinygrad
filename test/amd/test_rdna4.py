...
class TestCustomKernel(unittest.TestCase):
  ...
  def test_add(self):
    def test(C:UOp, B:UOp, A:UOp) -> UOp:
      ...
      e(s_load_b64(sdata=s[4:5], sbase=s[0:1], ioffset=ptr_offset(C), soffset=NULL))  # C (dest)
      e(s_load_b64(sdata=s[6:7], sbase=s[0:1], ioffset=ptr_offset(B), soffset=NULL))  # B (source)
      e(s_load_b64(sdata=s[8:9], sbase=s[0:1], ioffset=ptr_offset(A), soffset=NULL))  # A (source)
      e(s_wait_kmcnt(simm16=0))
      # Load from A and B
      e(global_load_b32(vdst=v[10], vaddr=v[0:1], saddr=s[8:9]))  # load A[0]
      e(global_load_b32(vdst=v[11], vaddr=v[0:1], saddr=s[6:7]))  # load B[0]
      e(s_wait_loadcnt(simm16=0))
      # Add: v[12] = A[0] + B[0] (use int add since tensors are int32)
      e(v_add_nc_u32_e32(vdst=v[12], src0=v[10], vsrc1=v[11]))
      # Store to C
      e(global_store_b32(vaddr=v[0:1], vsrc=v[12], saddr=s[4:5]))
      e(s_wait_storecnt(simm16=0))
      e(s_endpgm())
