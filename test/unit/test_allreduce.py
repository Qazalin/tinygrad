...
class TestRingAllReduce(unittest.TestCase):
  ...
  def test_correct_all2all_stack(self):
    with Context(ALL2ALL=2):
      ...
      for buf in mb.bufs: self.assertListEqual(list(buf.as_memoryview().cast("f")), [N]*N*96)
      ...
      for buf in cast_mb.bufs: self.assertListEqual(list(buf.as_memoryview().cast("H")), [0x4100]*N*96)
