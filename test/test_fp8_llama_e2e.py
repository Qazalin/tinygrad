...
def new_matmul(x:Tensor, w:Tensor, fp8=FP8, amax_x:Tensor|None=None, amax_w:Tensor|None=None) -> tuple[Tensor, ...]:
  ...
  return asm_gemm(x_fp8, w_fp8.T, combined_scale=combined_scale), x_new_amax, w_new_amax


if __name__ == "__main__":
  Tensor.manual_seed(0)
  x_rand = Tensor.randn(1, 8192, 14336, dtype=dtypes.bfloat16)
  w_rand = Tensor.randn(4096, 14336, dtype=dtypes.bfloat16)
  with Context(DEBUG=0):
    Tensor.realize(x_rand, w_rand)

  x_new, w_new = x_rand.clone().requires_grad_(), w_rand.clone().requires_grad_()
  y_new, x_amax_new, w_amax_new = new_matmul(x_new, w_new, fp8=True)
  ...
  y_ref, x_amax_ref, w_amax_ref = ref_matmul(x_ref, w_ref, fp8=True)
