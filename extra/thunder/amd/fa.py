...
def _fa_grad_fxn(B, H, N, D, H_local, H_KV_local, H_KV, B_local, shard_axis, shard_axis_t, single_device, arch, has_sink,
  ...
  def grad(dou:UOp, ker:UOp) -> tuple:
    ...
    if fp8_qk:
      ...
      assert asm_fp8 and pre_scaled_fp8, "production FP8 FA requires pre-scaled Q/K and the assembly kernel"
      v8, vs = input_tensor(8), input_tensor(9)
    ...
    dq, dk_partial, dv_partial = Tensor.custom_kernel(
      dq, dk_partial, dv_partial, do, xq, xk, xv, l_vec, delta_vec,
...
def flash_attention(xq, xk, xv, attn_mask:Tensor|None=None, is_causal:bool=False, write_flat:bool=False, sinks:Tensor|None=None,
  ...
  if getenv("FP8_FA") if fp8_qk is None else fp8_qk:
    ...
    assert asm_fp8, "only the final gfx950 FP8 assembly FA kernel is supported"
    if pre_scaled_fp8:
