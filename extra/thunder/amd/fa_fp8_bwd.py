...
def custom_fp8_backward(*args:UOp, B:int, N:int, H:int, H_KV:int, arch:str):
  assert arch == "gfx950" and (B, N, H, H_KV) == (2, 8192, 32, 8)
  assert args[0].dtype == dtypes.bfloat16
  ...
  Rhythm = tuple(UOp(Ops.INS, arg=(inst, dtypes.void)) for inst in build_kernel(B, N, H, H_KV))
  return UOp(Ops.PROGRAM, src=(sink, UOp(Ops.LINEAR, src=Rhythm)))
