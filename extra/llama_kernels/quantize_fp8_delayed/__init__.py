...
def quantize_fp8_delayed(x:Tensor, amax_state:Tensor, fp8_dtype=dtypes.fp8e4m3, return_inv_scale:bool=True) -> tuple[Tensor, Tensor|None, Tensor, UOp]:
  # NOTE: one-pass bf16 -> fp8 quantize with delayed scaling. Returns (fp8, optional inv_scale, new_amax, store_effect).
  ...
  inv_scale = (amax_state.float() + 1e-8) / FP8_MAX if return_inv_scale else None
