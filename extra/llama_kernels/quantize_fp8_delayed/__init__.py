...
def _custom_quantize_fp8_with_amax(fp8_out:UOp, inv_scale_out:UOp, amax_partial:UOp, x:UOp, amax_state:UOp) -> UOp:
  ...
  inv_scale_store = inv_scale_out.reshape(1)[0].store((amax_state[0].cast(dtypes.float) + 1e-8) / FP8_MAX)
  return UOp.sink(amax_store.end(tid, wg), inv_scale_store, arg=KernelInfo(f"quantize_fp8_with_amax_{n_elems}", opts_to_apply=()))
