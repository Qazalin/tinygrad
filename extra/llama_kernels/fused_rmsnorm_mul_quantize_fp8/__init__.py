...
def _custom_fwd(fp8_out:UOp, inv_scale_out:UOp, x_normed_out:UOp, rrms_out:UOp, amax_buf:UOp,
  ...
  mem = n_elems * 2 + 4 + n_elems + MBS * SEQ * 4 + n_elems + HIDDEN * 2 + NUM_WG * 4 + 4
...
def _custom_fwd_add(fp8_out:UOp, inv_scale_out:UOp, h_out:UOp, x_normed_out:UOp, rrms_out:UOp, amax_buf:UOp,
  ...
  n_elems = MBS * SEQ * HIDDEN
  threads, workgroups = UOp.special(THREADS_PER_WG, "lidx0"), UOp.special(NUM_WG, "gidx0")
  mem = n_elems * 2 * 4 + 4 + MBS * SEQ * 4 + HIDDEN * 2 + NUM_WG * 4 + 4
...
def _fused_add_bwd(*args, **kwargs):
  ...
  # Outputs: (fp8_out, inv_scale_out, h_out, x_normed_out, rrms_out, amax_buf). fp8 and h may be consumed
...
def fused_rmsnorm_mul_quantize_fp8(x:Tensor, weight:Tensor, amax_state:Tensor, inv_scale_out:Tensor, eps:float, fp8_dtype) -> tuple[Tensor, Tensor, Tensor, Tensor, Tensor]:
  ...
  fp8_out, inv_scale_out, x_normed_out, rrms_out, amax_buf, *_ = Tensor.custom_kernel(
    fp8_out, inv_scale_out, x_normed_out, rrms_out, amax_buf, x, weight, amax_state, fxn=fxn, grad_fxn=_fused_bwd)
  return fp8_out, inv_scale_out, scalar_amax(amax_buf), x_normed_out, rrms_out

def fused_add_rmsnorm_mul_quantize_fp8(x:Tensor, residual:Tensor, weight:Tensor, amax_state:Tensor, inv_scale_out:Tensor,
  ...
  fp8_out, inv_scale_out, h_out, x_normed_out, rrms_out, amax_buf, *_ = Tensor.custom_kernel(
    fp8_out, inv_scale_out, h_out, x_normed_out, rrms_out, amax_buf, x, residual, weight, amax_state,
    fxn=fxn, grad_fxn=_fused_add_bwd)
  return fp8_out, inv_scale_out, scalar_amax(amax_buf), h_out, x_normed_out, rrms_out
