...
@functools.cache
def _custom_quantize_mxfp4(q:UOp, e8:UOp, x:UOp, use_hadamard:bool, shuffle_data:bool, shuffle_scales:bool,
                           transpose:bool, dname:str) -> UOp:
  rows, K = x.shape
  n_blocks = rows * (K // BLK)
  ...
  defines = [f"-DROWS={rows}", f"-DK_DIM={K}", f"-DTHREADS_PER_WG={THREADS_PER_WG}", f"-DUSE_HADAMARD={int(use_hadamard)}",
             ...
             f"-DTRANSPOSE_INPUT={int(transpose)}"]
  return UOp(Ops.PROGRAM, src=(sink, UOp(Ops.LINEAR, src=(*sink.src, sink)),
                               UOp(Ops.SOURCE, arg=src), UOp(Ops.BINARY, arg=compile_hip(src, defines))),
             arg=quantize_mxfp4_program_info(sink))


def quantize_mxfp4(x:Tensor, use_hadamard:bool=False, shuffle_data:bool=False, shuffle_scales:bool=False,
                   transpose:bool=False) -> tuple[Tensor, Tensor]:
  ...
  rows, K = x.shape if not transpose else (x.shape[1], x.shape[0])
  assert K % BLK == 0, f"K={K} must be divisible by {BLK}"
  # A logical transpose moves a row-sharded tensor's shard to the column axis.
  axis = ((1 - x.uop.axis) if transpose and x.uop.axis is not None else x.uop.axis) if isinstance(x.device, tuple) else None
  ...
  fxn = functools.partial(_custom_quantize_mxfp4, use_hadamard=use_hadamard, shuffle_data=shuffle_data,
                          shuffle_scales=shuffle_scales, transpose=transpose, dname=dname_of(x.device))
