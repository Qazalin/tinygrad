...
def quantize_mxfp4(x:Tensor, *, shuffle_row:bool=False, shuffle_col:bool=False, flatten_row:bool=False,
  ...
  shapes = ((M, N//2) if flatten_row else (*x.shape[:-1], N//2),
            (M, N//32) if flatten_row else (*x.shape[:-1], N//32), (N, M//2), (N, M//32))
  if out is None:
    outputs = (alloc_like(shapes[0], dtypes.uint8, x.device, row_axis), alloc_like(shapes[1], dtypes.uint8, x.device, row_axis),
               alloc_like(shapes[2], dtypes.uint8, x.device, col_axis), alloc_like(shapes[3], dtypes.uint8, x.device, col_axis))
  else:
    assert len(out) == 4 and all(o.dtype == dtypes.uint8 and o.shape == shape and o.device == x.device for o, shape in zip(out, shapes)), \
      "cached MXFP4 outputs must match the quantized weight"
    outputs = out
  fxn = functools.partial(_custom_quantize_mxfp4, shuffle_row=shuffle_row, shuffle_col=shuffle_col)
  ret = tuple(Tensor.custom_kernel(*outputs, x, fxn=fxn)[:4])
  if out is not None:
