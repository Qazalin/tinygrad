...
def _build_c_src(n_chunks:int) -> str:
...
@functools.cache
def _custom_c_fused_pad_grad_accum(grad_buf:UOp, *chunk_uops, dname:str, n_chunks:int, chunk_size:int) -> UOp:
  ...
  sink = UOp.sink(grad_buf.base, *(c.base for c in chunk_uops), threads, workgroups,
                  arg=KernelInfo(f"fused_pad_grad_accum_n{n_chunks}_c{chunk_size}", estimates=Estimates(ops=2*total, mem=mem)))
  src = _build_c_src(n_chunks)
...
@functools.cache
def _custom_uop_fused_pad_grad_accum(grad_buf:UOp, *chunk_uops, n_chunks:int, chunk_size:int) -> UOp:
  total = n_chunks * chunk_size
  elems_per_block = THREADS_PER_WG * ELEMS_PER_THREAD
  assert chunk_size % elems_per_block == 0, f"chunk_size {chunk_size} must be multiple of {elems_per_block}"
...
def fused_pad_grad_accum(grad_buf:Tensor, chunks:list[Tensor]) -> Tensor:
  ...
  if getenv("FUSED_PAD_GRAD_ACCUM_CUSTOM_C"):
    fxn = functools.partial(_custom_c_fused_pad_grad_accum, dname=dname_of(grad_buf.device), n_chunks=len(chunks), chunk_size=chunk_size)
  else:
    fxn = functools.partial(_custom_uop_fused_pad_grad_accum, n_chunks=len(chunks), chunk_size=chunk_size)
