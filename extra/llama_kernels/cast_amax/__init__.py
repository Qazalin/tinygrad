...
from extra.llama_kernels import NUM_WG, THREADS_PER_WG, compile_cpp, alloc_like, alloc_local, zero_scalar, dname_of
...
def _custom_fused_bwd_w13(grad_xw13_fp8:UOp, grad_amax_next:UOp, grad_amax:UOp,
  ...
  sink = UOp.sink(grad_xw13_fp8.base, grad_amax_next, grad_amax.base,
...
@functools.cache
def _custom_fused_cast_amax_w13(fp8_out:UOp, amax_out:UOp, xw13:UOp, amax_state:UOp, grad_amax_state:UOp, next_grad_amax_state:UOp, dname:str) -> UOp:
...
def _fused_quantize_bwd_w13(gradient:UOp, kernel:UOp):
  ...
  grad_amax_next = zero_scalar(device)
...
def fused_quantize_fp8_w13(xw13:Tensor, amax_state:Tensor, fp8_dtype, grad_amax_state:Tensor, next_grad_amax_state:Tensor) -> tuple[Tensor, Tensor]:
  ...
  amax_out = zero_scalar(xw13.device)
