...
from tinygrad.helpers import ContextVar, getenv, prod
...
# module-level mailbox: grad_xw13 token UOp -> (grad_xw13_fp8 UOp, inv_scale UOp)
...
@functools.cache
def _custom_fused_bwd_w13_fp8_only(grad_xw13_fp8_out:UOp, grad_amax_buf:UOp,
                                   xw13:UOp, grad_x2:UOp, amax_state:UOp, grad_amax_state:UOp) -> UOp:
  VEC = 8
  n_elems = prod(grad_x2.shape)
  HIDDEN = grad_x2.shape[-1] if len(grad_x2.shape) > 1 else 14336
  ...
  stores = grad_xw13_fp8_out[row, 0, col].store(scaled_g1.cast(grad_xw13_fp8_out.dtype.base))
  stores = grad_xw13_fp8_out.after(stores)[row, 1, col].store(scaled_g3.cast(grad_xw13_fp8_out.dtype.base)).end(lane)
  amax_store = store_wg_amax(grad_amax_buf, abs_g, stores, axes)
  return amax_store.end(tid, wg).sink(arg=KernelInfo(f"fused_silu_mul_bwd_w13_fp8_only_{n_elems}", opts_to_apply=()))
...
def _fused_quantize_bwd_w13(gradient:UOp, kernel:UOp):
  ...
  if getenv("ASM_GEMM", 0):
    grad_xw13_fp8, grad_amax_buf, *_ = Tensor.custom_kernel(
      ...
      Tensor(amax_state, device=device), grad_amax_state_t, fxn=_custom_fused_bwd_w13_fp8_only)
    ...
    grad_xw13, grad_xw13_fp8, grad_amax_buf, *_ = Tensor.custom_kernel(
      ...
      Tensor(xw13, device=device), Tensor(gradient, device=device).cast(dtypes.bfloat16),
      Tensor(amax_state, device=device), grad_amax_state_t, fxn=_custom_fused_bwd_w13)
    grad_xw13_uop = grad_xw13.uop
