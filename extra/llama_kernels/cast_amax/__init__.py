...
@functools.cache
def _custom_fused_bwd_w13(grad_xw13_fp8:UOp, inv_scale_out:UOp, grad_amax_buf:UOp,
  ...
  sink = UOp.sink(grad_xw13_fp8.base, inv_scale_out.base, grad_amax_buf.base,
...
@functools.cache
def _custom_fused_cast_amax_w13(fp8_out:UOp, inv_scale_out:UOp, amax_buf:UOp, xw13:UOp, amax_state:UOp, grad_amax_state:UOp, grad_inv_scale_out:UOp, dname:str) -> UOp:
  ...
  sink = UOp.sink(fp8_out.base, inv_scale_out.base, amax_buf.base, xw13.base, amax_state.base, threads, workgroups,
...
def _fused_quantize_bwd_w13(gradient:UOp, kernel:UOp):
  _, _, _, xw13, amax_state, grad_amax_state, grad_inv_scale_out = kernel.src[1:]
  device = xw13.device
  axis = xw13.axis if isinstance(device, tuple) else None
  grad_xw13_fp8 = alloc_like(xw13.shape, dtypes.fp8e4m3,  device, axis)
  inv_scale_out = Tensor(grad_inv_scale_out, device=device)
  ...
  grad_xw13_fp8, inv_scale_out, grad_amax_buf, *_ = Tensor.custom_kernel(
    grad_xw13_fp8, inv_scale_out, grad_amax_buf,
  ...
  _grad_fp8_mailbox[grad_xw13_uop] = (grad_xw13_fp8_uop, inv_scale_out.uop)
  return (None, None, None, grad_xw13_uop, None, None, None)

def fused_quantize_fp8_w13(xw13:Tensor, amax_state:Tensor, fp8_dtype, grad_amax_state:Tensor, inv_scale_out:Tensor|None=None, grad_inv_scale_out:Tensor|None=None) -> tuple[Tensor, Tensor, Tensor]:
  ...
  if inv_scale_out is None: inv_scale_out = alloc_local((), dtypes.float32, xw13.device, axis)
  if grad_inv_scale_out is None: grad_inv_scale_out = alloc_local((), dtypes.float32, xw13.device, axis)
  ...
  fp8_out, inv_scale_out, amax_buf, *_ = Tensor.custom_kernel(fp8_out, inv_scale_out, amax_buf, xw13, amax_state, grad_amax_state, grad_inv_scale_out,
                                                                fxn=fxn, grad_fxn=_fused_quantize_bwd_w13)
  return fp8_out, inv_scale_out, scalar_amax(amax_buf)
