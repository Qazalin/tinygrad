...
def _custom_fused_bwd_w13(grad_xw13_fp8:UOp, inv_scale_out:UOp, grad_amax_buf:UOp,
  ...
  mem = n_elems * 2 * 3 + n_elems * 2 + 4 + NUM_WG * 4 + 4
...
@functools.cache
def _custom_fused_cast_amax_w13(fp8_out:UOp, inv_scale_out:UOp, amax_buf:UOp, xw13:UOp, amax_state:UOp, grad_amax_state:UOp, grad_inv_scale_state:UOp, dname:str) -> UOp:
  # NOTE: grad_amax_state is plumbed through as an unused fwd input so the bwd kernel can read it via kernel.src
  hidden = xw13.shape[2] // 2
  n_elems = xw13.shape[0] * xw13.shape[1] * hidden
  threads, workgroups = UOp.special(THREADS_PER_WG, "lidx0"), UOp.special(NUM_WG, "gidx0")
  mem = n_elems * 2 * 2 + n_elems + 4 + NUM_WG * 4
  sink = UOp.sink(fp8_out.base, inv_scale_out.base, amax_buf.base, xw13.base, amax_state.base, grad_inv_scale_state.base, threads, workgroups,
...
def _fused_quantize_bwd_w13(gradient:UOp, kernel:UOp):
  _, _, _, xw13, amax_state, grad_amax_state, grad_inv_scale_state = kernel.src[1:]
  device = xw13.device
  axis = xw13.axis if isinstance(device, tuple) else None
  grad_xw13_fp8 = alloc_like(xw13.shape, dtypes.fp8e4m3,  device, axis)
  inv_scale_out  = Tensor(grad_inv_scale_state, device=device)
...
def fused_quantize_fp8_w13(xw13:Tensor, amax_state:Tensor, inv_scale_out:Tensor, fp8_dtype, grad_amax_state:Tensor, grad_inv_scale_state:Tensor) -> tuple[Tensor, Tensor, Tensor]:
  ...
  fp8_out, inv_scale_out, amax_buf, *_ = Tensor.custom_kernel(fp8_out, inv_scale_out, amax_buf, xw13, amax_state, grad_amax_state, grad_inv_scale_state,
