...
def _mxfp4_shuffle_weight(x:Tensor) -> Tensor:
  # AITER shuffle_weight(x, layout=(16, 16)) on the packed uint8 buffer.
  rows, half_k = x.shape
...
def _mxfp4_shuffle_scales(x:Tensor) -> Tensor:
  # AITER e8m0_shuffle: each 256x8 scale tile is arranged for the raw MFMA scale loads.
...
def quantize_mxfp4(x:Tensor) -> tuple[Tensor, Tensor, Tensor]:
  ...
  # AITER "even" scale rounding: round the fp32 significand before choosing 2^(floor(log2)-2).
  ...
  # Round-to-nearest-even onto {0,.5,1,1.5,2,3,4,6}; comparisons encode the tie direction.
  mag = scaled.abs()
...
def asm_gemm(a:Tensor, b:Tensor, x_scale:Tensor|None=None, w_scale:Tensor|None=None, grad_amax_state:Tensor|None=None,
  ...
  if (is_mxfp4 := mxfp4):
    assert not mx and mx_scales is None, "mxfp4=True owns quantization; mx/mx_scales are for mxfp8"
    assert a.dtype in {dtypes.bfloat16, dtypes.float16}, f"cannot quantize {a.dtype} to mxfp4"
  counters["used"] += 1
