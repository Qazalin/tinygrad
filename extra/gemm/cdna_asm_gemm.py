...
def custom_gemm_bw(gradient:UOp, kernel:UOp, n_scales:int=2, has_grad_amax:bool=False, has_grad_inv_scale:bool=False,
                   has_w_post:bool=False, scale_mode:int|None=None):
  inputs = kernel.src[1:]
  if inputs[1].dtype == FP8_DTYPE:
    ...
    grad_inv_scale_state = inputs[i] if has_grad_inv_scale else None; i += has_grad_inv_scale
    ...
    if mailbox_entry is not None:
      ...
      if getenv("CURRENT_GRAD_SCALE", 0):
        ...
        g_fp8, g_scale, _, store_effect = quantize_fp8_delayed(g_t, Tensor(grad_amax_state, device=a.device), inv_scale_out=Tensor(grad_inv_scale_state, device=a.device))
    ...
    # dgrad: g_scale is passed through as the extra scale so the kernel applies the same delayed scaling path as forward.
...
def asm_gemm(a:Tensor, b:Tensor, x_scale:Tensor|None=None, w_scale:Tensor|None=None, grad_amax_state:Tensor|None=None,
             grad_inv_scale_state:Tensor|None=None, w_post_scale:Tensor|None=None, mx:bool=False, mx_scales:tuple|None=None,
             mx_w_stored:bool=False, extra_scale:Tensor|None=None) -> Tensor:
  ...
  if arch.startswith("gfx950") and getenv("USE_ASM", 1):
    ...
    # fp8 gemm computes a@b.T, kernel multiplies output by x_scale * w_scale before bf16 store
      ...
      bw = functools.partial(custom_gemm_bw, n_scales=len(scales), has_grad_amax=grad_amax_state is not None,
                             has_grad_inv_scale=grad_inv_scale_state is not None, has_w_post=w_post_scale is not None, scale_mode=scale_mode)
