...
def hk_fp8_atb_gemm(a:Tensor, b:Tensor, x_scale:Tensor|None=None, g_amax:Tensor|None=None, defer_k_allreduce:bool=False) -> Tensor:
...
def custom_gemm_bw(gradient:UOp, kernel:UOp, n_scales:int=2, has_grad_amax:bool=False, has_w_post:bool=False):
  ...
  if inputs[1].dtype == FP8_DTYPE:
    ...
    if getenv("USE_HK_FP8_ATB", 1):
      grad_b = hk_fp8_atb_gemm(g_fp8, a_t, x_scale=s_x_t, g_amax=g_amax, defer_k_allreduce=True)
    else:
...
@functools.cache
def _custom_gemm_bw_fxn(n_scales:int, has_grad_amax:bool, has_w_post:bool):
...
def asm_gemm(a:Tensor, b:Tensor, x_scale:Tensor|None=None, w_scale:Tensor|None=None, grad_amax_state:Tensor|None=None,
  ...
  if arch.startswith("gfx950") and getenv("USE_ASM", 1):
    ...
    # fp8 gemm computes a@b.T, kernel multiplies output by x_scale * w_scale before bf16 store
      ...
      bw = _custom_gemm_bw_fxn(len(scales), grad_amax_state is not None, w_post_scale is not None)
