...
def custom_asm_bf16_mlperf_gemm1(D:UOp, C:UOp, A:UOp, B:UOp, WS:UOp, Flags:UOp) -> UOp:
  ...
  sink = UOp.sink(d_write, A.flatten().index(zero).load(), B.flatten().index(zero).load(), ws_write, flags_write,
                  lds, threads, workgroups, arg=KernelInfo(f"asm_bf16_gemm1_{M}_{N}_{K}",
...
def _producer_mxfp4_outputs(gradient:UOp, expected_half_k:int) -> tuple[UOp, UOp, UOp|None, UOp|None]|None:
  ...
  for call in reversed(gradient.toposort()):
    ...
    if (isinstance(info, KernelInfo) and (info.name.startswith("swiglu_bwd_mxfp4_") or info.name == "fused_qkv_rope_backward_mxfp4")
    ...
    if (isinstance(info, KernelInfo) and info.name.startswith("rmsnorm_add_mul_bwd_mxfp4_row_")
        and call.src[3].shape[-1] == expected_half_k):
      assert len(call.src) >= 5
      return call.src[3].after(call), call.src[4].after(call), None, None
  return None
