...
def custom_hk_fp8_gemm(C:UOp, A:UOp, B:UOp, *args:UOp, dname:str, scale_mode:int=3, layer_scale:bool=False) -> UOp:
  ...
  sink_inputs = (C.base, A.base, B.base) + tuple(s.base for s in scales) + tuple(x.base for x in extra_args) + ((layer_num.base,) if layer_num is not None else ()) + (threads, workgroups)
  ...
  lib = HIPCCCompiler("gfx950", [f"-I{(kittens_path/'include').as_posix()}", "-std=c++20", "-DKITTENS_CDNA4", "-ffast-math",
                                 ...
                                 f"-DSCALE_MODE={scale_mode}", f"-DUNUSED_EXTRA_COUNT={len(extra_args)}", *( ["-DLAYER_SCALE=1"] if layer_scale else [] )]).compile_cached(src)
...
def hk_fp8_atb_gemm(a:Tensor, b:Tensor, x_scale:Tensor|None=None, g_amax:Tensor|None=None, defer_k_allreduce:bool=False, layer_num:Tensor|None=None) -> Tensor:
  ...
  out = Tensor.custom_kernel(out, a, b, *scales, *((layer_num,) if layer_num is not None else ()), fxn=functools.partial(custom_hk_fp8_atb_gemm, dname=dname, scale_mode=scale_mode, layer_scale=layer_num is not None))[0]
  if reduce_out:
  ...
  if inputs[1].dtype == FP8_DTYPE:
    ...
    if mailbox_entry is not None:
      ...
      if getenv("CURRENT_GRAD_SCALE", 0):
        ...
        store_effect = (next_grad_amax_state[Tensor(layer_num, device=a.device)[0]].store(new_grad_amax.uop) if layer_num_t is not None else next_grad_amax_state.store(new_grad_amax.uop))
    ...
    grad_a = asm_gemm(g_fp8, b_t, x_scale=s_x_t, w_scale=s_w_t, g_amax=g_amax, layer_num=layer_num_t) if has_w else asm_gemm(g_fp8, b_t, x_scale=s_x_t, g_amax=g_amax, layer_num=layer_num_t)
    # wgrad: no w_scale
    if getenv("DISABLE_HK_FP8_ATB", 0):
      g_fp8_T = g_fp8.permute(2, 0, 1).reshape(g_t.shape[-1], -1)
      grad_b = asm_gemm(g_fp8_T, a_t.reshape(-1, a_t.shape[-1]), x_scale=s_x_t, g_amax=g_amax, defer_k_allreduce=True, layer_num=layer_num_t)
    else:
    ...
    # one None per input: (out, a, b, x_scale[, w_scale][, grad_amax, next_grad_amax][, w_post_scale])
...
def _custom_gemm_bw_fxn(n_scales:int, has_grad_amax:bool, has_w_post:bool, has_layer_num:bool):
  return functools.partial(custom_gemm_bw, n_scales=n_scales, has_grad_amax=has_grad_amax, has_w_post=has_w_post, has_layer_num=has_layer_num)
...
def asm_gemm(a:Tensor, b:Tensor, x_scale:Tensor|None=None, w_scale:Tensor|None=None, grad_amax_state:Tensor|None=None,
             ...
             w_post_scale:Tensor|None=None, mx:bool=False, mx_scales:tuple|None=None, mx_w_stored:bool=False, g_amax:Tensor|None=None, layer_num:Tensor|None=None) -> Tensor:
