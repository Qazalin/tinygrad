...
from extra.gemm.asm.arg import KernelArgs
...
if getenv("ASM", 1): # re assemble
  ...
  with open(pathlib.Path(__file__).parent/"lib", "rb") as f: lib:bytes = f.read()
  name:str = "Custom_Cijk_Alik_Bljk_BBS_BH_MT256x256x64_MI16x16x1_UserArgs_shortname0_gfx950"
...
# Pointers
  args.D = bufs[0].value
  args.C = bufs[0].value      # safe when beta=0
  args.A = bufs[1].value
  args.B = bufs[2].value

# Strides (in elements)
# Interpret kernel's A(l,i) as A[i,l] in row-major storage, same for B(l,j) as B[j,l]
  args.strideD0 = N
  args.strideD1 = N * N
...
prg.vargs = extra
et = prg(global_size=[GX, 1, 1], local_size=[256, 1, 1], wait=True)
