...
def aiter_fhma_grad_fxn(gradient:UOp, kernel:UOp) -> tuple[None, UOp, UOp, UOp, None]:
  dname = gradient.device
  out, q_perm, k_perm, v_perm, lse = [Tensor(u, device=u.device) for u in kernel.src[1:]]
  B, S, H, D = q_perm.shape
  delta = Tensor.empty((B, H, S), dtype=dtypes.float32)
  ...
  dq_acc = Tensor.empty((1, B, H, S, D), dtype=dtypes.float32)
  dq_acc = Tensor.custom_kernel(dq_acc, fxn=_zero_kernel)[0]

  dk = Tensor.empty((B, S, H, D), dtype=dtypes.bfloat16)
  dv = Tensor.empty((B, S, H, D), dtype=dtypes.bfloat16)
  ...
  dq = Tensor.empty((B, S, H, D), dtype=dtypes.bfloat16)
...
def asm_atn(q:Tensor, k:Tensor, v:Tensor, **kwargs) -> Tensor:
  ...
  out = Tensor.custom_kernel(out, q_perm, k_perm, v_perm, lse, fxn=functools.partial(aiter_fmha_fwd, dname=dname), grad_fxn=aiter_fhma_grad_fxn)[0]
