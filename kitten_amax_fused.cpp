// kitten_amax_fused: one-kernel amax reduction
// each workgroup scans multiple 16x32 tiles and atomically updates one global max
...
extern "C" __global__ void kitten_amax_fused(float *amax_out, const bf16 *x) {
  ...
  if (laneid() == 0) atomicMaxFloatNonNeg(amax_out, block_max);
}
